      ...
      <div class="form-item__content chip-btn__reg">
        <div class="form-item-row">
          <div class="input-item select-button-type">
            <span class="radiobox">
              <input type="radio" name="btnEvent" id="url" value="urlExternal" checked v-model="button.btnEvent"/>
              <label for="url"><span class="radiobox__text">URL 연결</span></label>
            </span>
            <span class="radiobox">
              <input type="radio" name="btnEvent" id="call" value="call" v-model="button.btnEvent"/>
              <label for="call"><span class="radiobox__text">전화하기</span></label>
            </span>
            <span class="radiobox">
              <input type="radio" name="btnEvent" id="otherType" value="other" v-model="button.btnEvent"/>
              <label for="otherType"><span class="radiobox__text">기타</span></label>
            </span>
            <div class="input">
              <Dropdown :options="buttonEventOptions" placeholder="선택" v-model="button.btnEvent"
...
export default {
  ...
  mounted () {
    this.$watch('buttonLen', function(newVal, oldVal) {
      console.log(newVal, this.buttonList.length)
      this.buttonCont = this.buttonList.length
      if (this.buttonCont < newVal) {
        const diff = newVal - this.buttonCont
        for (let i = 0; i < diff; i++) {
          this.buttonList.push(i)
        }
      } else if (this.buttonCont > newVal) {
        const diff = this.buttonCont - newVal
        this.buttonList.splice(this.buttonCont - diff, diff)
      }
    })
  },
  methods: {
