<template>
  <table class="table table-bodyonly form-table">
    ...
    <tbody>
      <tr v-for="(thum, i) in info" :key="i">
        <th scope="row"><span class="form-item__label required">상세 이미지 {{ i+1 }}</span></th>
        <td>
          <div class="form-item__content">
            <div class="form-item-row">
              <div class="input-item">
                <span class="input">
                  <input type="text" class="input" :value="thum.imgFile" disabled>
                </span>
              ...
              <div class="input-item input-limit input-url">
                <div class="input">
                  <input type="text" placeholder="http로 시작하는 URL 주소를 입력해주세요." v-model="thum.imgUrl"
                  />
                </div>
              </div>
            </div>
