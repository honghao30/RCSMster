            ...
            <div class="input-item input-limit w--full" >
              <div class="input">
                ...
                <div class="input-limit__text">
                  <p :style='buttonInfo.btnName.length > componentStrSizeRule["btnName1"] ? "color:red;" : ""'>
                    {{ buttonInfo.btnName.length }}/{{ componentStrSizeRule['btnName1'] }}자
                  </p>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    componentStrSizeRule() {
      ...
      if (this.componentParam) {
        for (const obj of this.componentParam) {
  ...
  methods: {
    ...
    calcText() {
      this.calcLength(this.buttonInfo.btnName)
    },
    calcLength(text) {
      this.btnNameLength = text.split(/{{.*?}}/).join('').length
    }
  }
}
</script>
