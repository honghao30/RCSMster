<template>
  <div class="form-item__content">
    <div class="form-item-row">
      <div class="input-item ip" v-for="(addAllowIp, index) in addAllowIpList" :key="addAllowIp.namekey">
        <!-- 기등록 항목 표시 -->
        ...
        <template v-else-if="edit && addAllowIp.regNo == null">
          <span class="input">
            <input type="text" class="input" placeholder="단일 IP 주소 입력" maxlength="15" :ref="addAllowIp.allowIpkey" v-model.trim="addAllowIp.allowIp">
          </span>
          <span class="input">
            <input type="text" class="input" placeholder="항목명" maxlength="10" :ref="addAllowIp.namekey" v-model.trim="addAllowIp.name">
          </span>
          <ButtonCmp type="btn-default-line" size="small" @click="removeAllowIp(index)">삭제</ButtonCmp>
        </template>
      </div>
      <ButtonCmp type="btn-default-line" size="small" v-if="edit && addAllowIpList.length < 10" @click="addAllowIp()">추가</ButtonCmp>
    </div>
  </div>
</template>
