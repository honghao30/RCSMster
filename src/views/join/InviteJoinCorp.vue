<template>
  <div class="join">
    ...
    <!-- 회원정보 입력 -->
    <JoinUserInfo v-show="stepActiveIndex === 2" :userType="this.userType" :corpId="corpInfo.corpData.corpId" :paramEmail='email' ref="userInfo" @changeUserInfo="changeUserInfo" />

    <!-- 가입 완료 -->
    <JoinComplete v-show="stepActiveIndex === 3" :viewType="this.viewType" />

    <div class="button__wrap">
      <ButtonCmp type="btn-blue-line" v-if="stepActiveIndex === 1" @click="cancelStep">취소</ButtonCmp>
      <ButtonCmp type="btn-blue-line" v-if="stepActiveIndex === 2" @click="prevStep">이전</ButtonCmp>
      <ButtonCmp type="btn-blue" v-if="stepActiveIndex === 1" :disabled="nextStepValidation" @click="nextStep">다음</ButtonCmp>
      <ButtonCmp type="btn-blue" v-if="stepActiveIndex === 2" :disabled="applyJoinValidation" @click="beforeApplyJoin">신청</ButtonCmp>
    </div>
  </div>
</template>
...
import { createJoinCorp, tokenCerti } from '@/api/join/applyJoin'
...
export default {
  ...
  data() {
    return {
      stepTitle: ['약관동의', '회원정보 입력', '운영자 가입 완료'],
      stepActiveIndex: 1,
      userType: 'CORP',
      termInfo: [],
      termInfoPass: false,
  ...
  methods: {
    ...
    tokenCerti() {
      ...
      tokenCerti(formData).then((res) => {
        if (res.code === '20000000') {
          this.corpInfo.corpData = res.result.corp
          this.brandInfo = res.result.brand
