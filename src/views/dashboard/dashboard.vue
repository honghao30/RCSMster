<template>
  <div class="dashboard__wrap">
    <div class="dashboard__wrap--inner">
        <!-- <DashBaordLnb /> -->
        <DashBoardMain :corpInfo="corpInfo" :isShowFavoriteGuide.sync='isShowFavoriteGuide' :isShowBrandInviteGuide.sync='isShowBrandInviteGuide' />
        <DashBoardSide :corpInfo="corpInfo" :isShowFavoriteGuide='isShowFavoriteGuide' :isShowBrandInviteGuide.sync='isShowBrandInviteGuide' />
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      isShowFavoriteGuide: false, // 즐겨찾기 가이드 노출 여부
      isShowBrandInviteGuide: false // 브랜드 운영권한 초대 가이드 노출 여부
    }
  },
  created() {
  ...
  methods: {
    getCorpInfo() {
      ...
      getCorpInfo(reqData).then(res => {
        ...
        this.corpInfo.saftyStatus = res.result.saftyStatus
      })
    }
  }
}
