<template>
  <div class="dv2023_autoreply-chips">
    <div class="rcs_item_bundle">
      <strong class="rcs_tit_bundle">칩리스트</strong>
      <div v-if="!viewMode" class="rcs_cont_bundle">
        <span class="custom_radio checked min" @click="clickChipYes">
          <input type="radio" name="isChip" id="isChipYes" value="Y" v-model="chipYn" :disabled="templateInfo.tplId === undefined"/>
          <label for="isChipYes"><span>사용</span></label>
        </span>
        <span v-if="this.chipYn == 'Y'"  class="custom_select micromin">
          <select v-model="countSuggestion" @change="changeCountSuggestion">
            <option v-for="index in templateInfo.maxChipListCnt" :key="`suggChip-${index}`" :value="index">
                {{ index}}
            </option>
          </select>
        </span>
        <span class="custom_radio checked min">
          <input type="radio" name="isChip" id="isChipNo" value="N" v-model="chipYn" @click="clearCount" :disabled="templateInfo.tplId === undefined"/>
          <label for="isChipNo"><span>미사용</span></label>
        </span>
      </div>
      <div v-else class="rcs_cont_bundle">{{labelChip}}</div>
    </div>

    <!-- 2021.08.09 RBC고도화수정1 -->
    <div v-if="countSuggestion > 0" class="rcs_item_bundle">
      <strong class="rcs_tit_bundle"></strong>
      <div class="rcs_cont_bundle">
        <!-- 칩 -->
        <div v-for="(sugg, index) in autoReplyData.chipSuggestions" :key="`chipList_${index}`" class="box_table">
          <div class="box_th">
            <strong class="title">칩{{index + 1}}</strong>
          </div>
          <div class="box_td">
            <div class="box_btn_action_bundle">
              <div class="box_top_bundle">
                <span class="custom_select">
                  <select v-model="sugg.type" :ref="`action_${index}_type`" :id="`chip_action_${index}`" @change="handleChangeAction($event, index)" :disabled="viewMode" required>
                ...
                <span class="custom_input">
                  <input type="text" :ref="`action_${index}_displayText`" placeholder="칩명 입력" v-model.trim="sugg.action.displayText" :disabled="viewMode" maxlength="17" @input="e => sugg.action.displayText = e.target.value">
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.displayText) }}</span>/17)</p>
                </span>
              </div>
              <!-- 복사하기-->
              <div v-if="sugg.type === 'copy_to_clipboard'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.clipboardAction.copyToClipboard.text) }}</span>/200)</p>
                </span>
              </div>
              <!-- URL연결(외부)-->
              <div v-if="sugg.type === 'open_url'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  <input type="text" :ref="`action_${index}_url`" placeholder="URL 입력" v-model.trim="sugg.action.urlAction.openUrl.url" :disabled="viewMode" maxlength="100" @input="e => sugg.action.urlAction.openUrl.url = e.target.value">
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.urlAction.openUrl.url) }}</span>/100)</p>
                </span>
              </div>
              <!-- URL연결(내부)-->
              <div v-if="sugg.type === 'local_url'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.localBrowserAction.openUrl.url) }}</span>/100)</p>
                </span>
                <span class="custom_input">
                    <span style="padding-right:10px">Half View</span>
                    <span class="custom_radio checked min">
                      <input type="radio" :id="`chip_${index}_isHalfView_Y`" v-model="sugg.action.localBrowserAction.openUrl.isHalfView" value=true :disabled="viewMode">
                      <label :for="`chip_${index}_isHalfView_Y`"><span>사용</span></label>
                    </span>
                    <span class="custom_radio checked min">
                      <input type="radio" :id="`chip_${index}_isHalfView_N`" v-model="sugg.action.localBrowserAction.openUrl.isHalfView" value=false :disabled="viewMode">
                      <label :for="`chip_${index}_isHalfView_N`"><span>미사용</span></label>
                    </span>
                </span>
...
“name”:”value”, “age”:30, “isBoolean”: true"
              ...
              <div v-if="sugg.type === 'dial_phone_number'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.dialerAction.dialPhoneNumber.phoneNumber) }}</span>/40)</p>
                </span>
              </div>
              <!-- 지도보여주기(좌표)-->
              <div v-if="sugg.type === 'show_location'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.mapAction.showLocation.location.latitude) }}</span>/40)</p>
                </span>
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.mapAction.showLocation.location.longitude) }}</span>/40)</p>
                </span>
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.mapAction.showLocation.location.label) }}</span>/100)</p>
                </span>
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.mapAction.showLocation.fallbackUrl) }}</span>/100)</p>
                </span>
              </div>
              <!-- 지도보여주기(쿼리)-->
              ...
              <div v-if="sugg.type === 'create_calendar_event'" class="box_bottom_bundle active">
                <div class="cont_flex_bundle date_checkbox">
                  ...
                  <span class="custom_checkbox" style="display: none !important;">
                    <!-- 변수사용 불가 -->
                    <!-- <input type="checkbox" :id="`chip_action_${index}_useVariable`" v-model="sugg.action.calendarAction.createCalendarEvent.useVariable" @change="clearTime(sugg.action.calendarAction.createCalendarEvent)" :disabled="viewMode">
                    <label :for="`chip_action_${index}_useVariable`">변수</label> -->
                  </span>
                </div>
                <span class="custom_input dv2023_countbumper" :style="sugg.action.calendarAction.createCalendarEvent.useVariable ? `margin-top: 10px;` : ``">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.calendarAction.createCalendarEvent.title) }}</span>/100)</p>
                </span>
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.calendarAction.createCalendarEvent.description) }}</span>/500)</p>
                </span>
              </div>
              <!-- 메시지작성-->
              <div v-if="sugg.type === 'compose_text_message'" class="box_bottom_bundle active">
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.composeAction.composeTextMessage.phoneNumber) }}</span>/40)</p>
                </span>
                <span class="custom_input dv2023_countbumper">
                  ...
                  <p class="dv2023_limitcount" v-if="!viewMode">(<span>{{ countDisplayTextLength(sugg.action.composeAction.composeTextMessage.text) }}</span>/100)</p>
                </span>
              </div>
            </div>
          </div>
...
export default {
  name: 'AutoReplyChipListSuggestion',
  ...
  watch: {
    ...
    chipYn: function(newVal, oldVal) {
      if (newVal === 'Y' && this.countSuggestion === 0) {
        this.countSuggestion = 1
  ...
  methods: {
    clickChipYes() {
      if (jglib.isEmpty(this.templateInfo.tplId)) {
        this.$alertMsg('템플릿을 선택해 주세요.')
      }
    },
    init() {
      this.countSuggestion = jglib.isEmpty(this.autoReplyData.chipSuggestions) ? 0 : this.autoReplyData.chipSuggestions.length

      if (this.countSuggestion > 0) this.chipYn = 'Y'

      // 캘린더 값이 있으면 TimeZone에 맞게 변경해준다.
      if (this.autoReplyData.chipSuggestions) {
        this.autoReplyData.chipSuggestions.forEach((sugg) => {
          if (
    ...
    changeCountSuggestion() {
      ...
      if (this.autoReplyData.chipSuggestions.length < this.countSuggestion) {
        let diff =
          this.countSuggestion - this.autoReplyData.chipSuggestions.length
        for (let i = 0; i < diff; i++) {
          this.autoReplyData.chipSuggestions.push(
            JSON.parse(JSON.stringify(defaultRefData['suggestions']['default']))
          )
        }
      } else {
        let diff =
          this.autoReplyData.chipSuggestions.length - this.countSuggestion
        for (let i = 0; i < diff; i++) {
          this.autoReplyData.chipSuggestions.pop()
        }
      }
    },
    clearCount() {
    ...
    handleChangeAction(e, index) {
      if (e.target.value) {
        ...
        let action = {
          displayText:
            this.autoReplyData.chipSuggestions[index].action.displayText,
          postback: {
            data: ''
          }
        }
        this.autoReplyData.chipSuggestions[index].action = Object.assign(
          defData,
          action
        )
        this.autoReplyData.chipSuggestions[index].type = e.target.value
      } else {
        this.autoReplyData.chipSuggestions[index].type = undefined
        this.autoReplyData.chipSuggestions[index].action = {
          displayText:
            this.autoReplyData.chipSuggestions[index].action.displayText,
          postback: {
            data: ''
    ...
    validateData() {
      for (let i = 0; i < this.autoReplyData.chipSuggestions.length; i++) {
        const sugg = this.autoReplyData.chipSuggestions[i]
        if (jglib.isEmpty(sugg.type)) {
          this.$alertMsg('칩 종류를 선택해 주세요.')
            .then((res) => {
    ...
    validateSuggestion() {
      let focus = null
      let suggestions = this.autoReplyData.chipSuggestions
...
<style>
.dv2023_autoreply-chips {
  border-top: 1px solid #d9d9d9;
  padding-top: 20px;
}
.dv2023_ar-write .custom_select.micromin {
  width: 112px;
  padding-right: 12px;
  border-right: 1px solid #d9d9d9;
}
.dv2023_ar-write .custom_select.micromin::after {
  right: 27px;
}
.dv2023_ar-write .custom_select.micromin select {
  min-width: 100px;
}
</style>
